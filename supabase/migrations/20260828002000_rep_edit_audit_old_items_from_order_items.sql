-- Migration: 20260828002000_rep_edit_audit_old_items_from_order_items.sql
-- Description: Fix the blank "before" column for services in the order-edit audit.
--
-- Why: rep_update_order_fields_with_audit snapshotted the previous services from
-- orders.items, the legacy JSON mirror. Orders created through the order_items
-- table keep that column at '[]' (759 of 1176 rows in production), so an edit
-- that touched the services recorded oldValues.items = [] and the review centre
-- rendered an empty "قبل" next to a populated "بعد".
--
-- The snapshot now reads the order_items rows and only falls back to the legacy
-- column, mirroring dbToOrder() on the client. Only that snapshot changed — the
-- rest of the function is identical to migration 091, which was verified against
-- the live definition before this migration was written.
--
-- This cannot repair events already written: the pre-edit service lines for those
