-- Migration 095: Fix sync_workflow_states_fn to handle Returned for Adjustments transitions correctly.
--   1. Ensure that if NEW.status = 'Returned for Adjustments', production_status transitions to 'in_production'.
--   2. Ensure that if NEW.status = 'Ready' and NEW.delivery_type = 'TryIn', it only transitions to 'try_in_ready' if OLD.issue_state was NOT 'returned'.
