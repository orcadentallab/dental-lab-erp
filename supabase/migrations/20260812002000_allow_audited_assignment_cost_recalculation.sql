-- Allow the audited representative edit RPC to persist the automatic cost
-- recalculation that accompanies a supplier/designer reassignment.
-- Standalone financial edits remain forbidden.
...
DO $$
DECLARE
    v_definition TEXT := pg_get_functiondef('public.orders_role_field_guard()'::regprocedure);
BEGIN
    IF v_definition NOT LIKE '%NEW.supplier_id IS NOT DISTINCT FROM OLD.supplier_id%'
       OR v_definition NOT LIKE '%NEW.designer_id IS NOT DISTINCT FROM OLD.designer_id%'
       OR v_definition NOT LIKE '%representative cannot change protected finance or identity fields%' THEN
        RAISE EXCEPTION 'Audited assignment cost recalculation guard was not installed';
    END IF;
END;
$$;
