-- Migration: Doctor Retention Dashboard and Settings
-- Date: 2026-07-06
-- Description: Creates settings, follow-up tables, indices, and the dynamic segmentation RPC.
...
CREATE TABLE IF NOT EXISTS doctor_retention_settings (
    ...
    one_case_churn_days INT NOT NULL DEFAULT 14,
    new_client_days INT NOT NULL DEFAULT 30,
    recently_churned_min_days INT NOT NULL DEFAULT 31,
    long_term_churn_days INT NOT NULL DEFAULT 90,
    decline_threshold_pct NUMERIC(5,2) NOT NULL DEFAULT 30.00,
    growth_threshold_pct NUMERIC(5,2) NOT NULL DEFAULT 15.00,
...
CREATE TABLE IF NOT EXISTS doctor_follow_ups (
    ...
    notes TEXT NOT NULL,
    status TEXT NOT NULL, -- 'promised_order', 'not_interested', 'busy', 'has_complaint', 'no_response'
...
CREATE POLICY doctor_follow_ups_select ON doctor_follow_ups 
    ...
    USING (
        get_my_role() = 'admin'
        OR (get_my_role() = 'representative' AND EXISTS (
            SELECT 1 FROM doctors 
            WHERE doctors.id = doctor_follow_ups.doctor_id 
...
BEGIN
    ...
    doctor_metrics AS (
        ...
        WHERE 
            -- عزل المندوب في حالة صلاحية المندوب
            (v_my_role != 'representative' OR d.representative_id = v_my_user_id)
