-- Migration: Sync items to order_items table on representative order edit and admin approval
-- =========================================================================================
...
BEGIN
    ...
    IF p_action = 'approve' THEN
        ...
        IF v_changes ? 'items' THEN
            ...
            IF jsonb_array_length(v_changes->'items') > 0 THEN
                INSERT INTO order_items (
                    order_id,
                    product_type,
                    teeth_numbers,
                ...
                FROM jsonb_array_elements(v_changes->'items') t(x);
            END IF;
        END IF;
