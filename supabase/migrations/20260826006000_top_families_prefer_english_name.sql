-- =====================================================================
-- Analytics label the family in English, not Arabic.
-- =====================================================================
--
-- Owner decision (2026-08-27): reports are read in English, while the
-- catalogue itself stays maintained in Arabic. So the reporting surfaces --
-- this RPC, the service-analysis tab and the profitability report -- show
-- name_en, and everything operational (order entry, the route editor, the
-- services catalogue) keeps showing name_ar.
--
-- name_en is nullable, so it cannot simply replace name_ar. The chain is:
--   name_en -> name_ar -> the raw service name
-- An English name that is present but blank (the UI writes NULL for an
-- empty field, but a whitespace-only value would survive a paste) has to
-- fall through as well, hence NULLIF(btrim(...), '') rather than a bare
-- COALESCE -- otherwise the card would render an empty label, which reads
-- as a bug rather than as missing data.
--
-- Grouping happens on the resolved label, so two families sharing an
-- English name would merge into one row. That is what a unique name_en
-- would prevent, and it is deliberately NOT added here: the column is
-- optional, and NULL is the common case today (zero families exist).
-- =====================================================================
...
-- Restated so the intended grant is readable from this file alone; see
-- 20260826002000 for why the privileged body must stay unreachable.
