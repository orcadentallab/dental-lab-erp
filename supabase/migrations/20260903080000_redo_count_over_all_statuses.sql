-- =====================================================================
-- Migration: count redos over every status, not only the closed ones
-- File: supabase/migrations/20260903080000_redo_count_over_all_statuses.sql
--
-- 20260903050000 rebuilt SECTION A1 around the doctor-statement settlement
-- scope, which is right for "how many cases closed this period" -- but it also
-- pulled redo_count and redo_cost into that scope, and a redo does not wait
-- for settlement to exist. A redo raised and still on the bench stopped being
-- counted at all, which analytics_summary_rejections_and_aging.test.sql caught
-- ("is_redo order is counted in redo_count regardless of its current status").
--
-- Those two figures now come from their own scope: every non-deleted order
-- whose statement date falls in the window, whatever its status. Everything
-- else in the function is unchanged from 20260903050000.
-- =====================================================================
...
BEGIN
    ...
    all_orders_in_range AS (
        -- redo_count / redo_cost describe the redo burden itself, which exists
        -- from the moment a redo is raised -- long before it settles. Scoping
        -- them to closed cases (as 20260903050000 did) silently dropped every
        -- redo still on the bench.
        SELECT o.status, o.is_redo, o.cost
        FROM orders o
        WHERE COALESCE(o.is_deleted, false) = false
          AND (
    ...
    SELECT jsonb_build_object(
        ...
        -- Counted over every status, not just the closed ones: a redo is a redo
        -- from the day it is raised.
        'redo_count',            COALESCE((SELECT COUNT(*) FROM all_orders_in_range WHERE is_redo = true), 0),
        -- Cancelled / Lab Rejected were never produced: zero cost, always.
        'redo_cost',             COALESCE((SELECT SUM(cost) FROM all_orders_in_range WHERE is_redo = true AND status NOT IN ('Cancelled', 'Lab Rejected')), 0),
