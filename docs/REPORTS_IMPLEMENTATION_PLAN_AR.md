...
>
> **آخر تحديث:** 2026-08-16 — **كل المراحل 0 لـ 6 خلصت.** المرحلة 7 مؤجلة رسمياً.
>
> ⚠️ **قبل أي شغل جديد:** `supabase db push` — فيه 5 مايجريشن جديدة من يوم 2026-08-16
> (`20260816003000` لـ `20260816007000`). من غيرها صفحات الربحية والتصنيف والتدفق النقدي
> وأداء المعامل والـ CAC هترمي `function does not exist`.
...
| `get_supplier_issue_performance` | `(DATE, DATE)` |
| `get_marketing_acquisition` | `(DATE, DATE)` |
...
## المرحلة 5 — ✅ خلصت (2026-08-16)

- **5.1 أداء المعامل** اتنقل لتقرير المشكلات — لكن **اتبنى من جديد** مش اتنسخ. القديم في
  `Quality.tsx` كان بيحسب الرفض من `orders.status` و`wasRejected()`، فالمشكلة اللي اتحلّت
  كانت بتختفي خالص: معمل صلّح مشاكله كان شكله زي معمل ماحصلش عنده مشاكل أصلاً.
  الـ RPC الجديدة `get_supplier_issue_performance` بتاخد البسط والمقام من `order_issues`
  على **محور تاريخ واحد** (تاريخ الطلب) — الخلط بين المحورين كان سبب 4 من 9 اختلافات في
  المرحلة 0. صف «داخلي / غير محدد» بيتعرض ما بيتشالش.
- **5.2 فورم المشكلة** اتعمل في commit سابق (الـ 14 كود + المرحلة المسؤولة + التصحيح والإلغاء الموثَّق).
- **5.3 صفحة الجودة اتحذفت** — مش بس redirect. مودال التقييم اللي فيها كان **مكرر** من
  `OrderList.tsx`، فمفيش قدرة اتشالت. `/quality` بقى redirect لـ `/issues-report`.
- **5.4 إحصائيات المصممين** بقت «إنتاجية الفريق» بتبويبين، وتبويب «مراحل الإنتاج» حالة
  فاضية بتشرح الناقص. عمود «يونت لكل 1000 ج.م راتب» بيظهر **للمصمم بالراتب الثابت بس** —
  المصمم بالقطعة مالوش تكلفة ثابتة تتقسم عليها.
- **5.5 القائمة الجانبية** أيقونة مختلفة لكل صفحة (3 صفحات كانت بتستخدم `BarChart3`).
  **ملاحظة:** `/aging-report` سيبناه في مجموعة «المالية» مكانه الحالي بدل ما نكرّر لينك ليه
  في التقارير، و«تصنيف العملاء» مالوش لينك مستقل لأنه بقى تبويب جوّه الربحية.

### ~~الوصف الأصلي للمرحلة 5~~
...
## المرحلة 6 — ✅ خلصت (2026-08-16)

- **6.1 التدفق النقدي** — `src/pages/CashFlow.tsx` · `/reports/cashflow` · 13 أسبوع فعلي
  بأسبوع بيبدأ الاثنين. **مفيش RPC جديدة** — التصنيف بـ `normalizeExpenseCategory()` في
  العميل زي ما الخطة طلبت. الرصيد الافتتاحي لأول أسبوع = كل الحركة قبل النافذة مش صفر.
  الأسبوع الفاضي بيتعرض كصف فاضي، لأن الصف الناقص بيتقري «مفيش بيانات اتحمّلت» مش «مفيش حركة».
  **الجزء التنبؤي مؤجّل ومكتوب في الواجهة صراحةً** إنه محتاج إدخال البنود المتوقعة.
- **6.2 اللوجستيات** قسم جوّه صفحة التدفق النقدي. نسبة الالتزام محسوبة على الطلبات اللي
  ليها **موعد وتاريخ تسليم فعلي معاً**، والباقي **مستبعد مش محسوب ملتزم** — والعدد معروض.
  متوسط تكلفة التوصيلة `null` لو مفيش صرف شحن، مش صفر.
- **6.3 CAC** — `src/components/reports/AcquisitionSection.tsx` جوّه `/marketing-analytics`
  بفترة **مستقلة** (شهر/3/6/12) لأن باقي الصفحة بتشتغل بالساعة واليوم، وطبيب اتسجّل النهاردة
  ما لحقش يطلب. **مفيش جدول `marketing_spend`** زي ما الخطة قالت.

### 🔴 قرار مهم خرج من 6.3 — مين بيصنّف المصروف

الـ RPC `get_marketing_acquisition` بترجّع المصروفات **بالفئة الخام مش مفلترة**، والعميل هو
اللي بيصنّف بـ `normalizeExpenseCategory()`. السبب: الدالة دي فيها قائمة aliases وتوحيد
عربي (الهمزات، ة/ه، ى/ي، التشكيل) للبيانات القديمة. تكرار المنطق ده في SQL كان هيعمل
**مصنّف تاني** يتفرّق عن الأول بصمت.

> **القاعدة:** أي RPC محتاجة تفلتر بفئة مصروف — ترجّع الفئات الخام وتسيب التصنيف للعميل.

**الـ CAC مقامه الأطباء المفعّلين مش المسجّلين** — طبيب اتسجّل وما طلبش كلّفنا فلوس وما جابش
شغل، والقسمة على المسجّلين كانت هتقلّل الرقم بالغلط. ولو الصرف صفر الـ CAC **مش معروض** خالص.

### ~~الوصف الأصلي للمرحلة 6~~
