...
## المرحلة 4 — الصفحات الجاهزة (داتا كاملة)
...
### مهمة 4.2 — الذمم: وسّع `/aging-report`، متبنيش جديد
**الملف:** `src/pages/AgingReport.tsx` (موجود — 1220 سطر)

الناقص عن المطلوب تلات حاجات بس:
1. **شريحة +60 مقسومة** لـ `61-90` و `+90` — المطلوب 4 شرائح، الموجود 3
2. **حد الائتمان** — عمود `credit_limit` من `entity_billing_settings` (اتضاف في `20260816001000`) مع مؤشر تجاوز
3. **DSO** — محسوب بالفعل في `Analytics.tsx`، ينقل هنا أو يتشارك
...
### مهمة 4.3 — تصنيف العملاء A/B/C/D — ⚠️ محتاج قرار المالك
