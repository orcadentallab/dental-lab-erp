...
const getOrderDisplayValue = getDoctorOrderDisplayAmount;

const ZERO_ACCOUNTING_STATUSES = new Set([
    'rejected',
    'doctor rejected',
    'lab rejected',
    'cancelled',
...
const isZeroValueStatementItem = (item: StatementItem): boolean => (
    ZERO_ACCOUNTING_STATUSES.has((item.status || '').trim().toLowerCase())
    && (item.officialAmount ?? item.amount) === 0
);
...
export default function Accounts() {
    ...
    const getEffectiveDesignPrice = useCallback((o: Partial<Order>) => {
    ...
    }, [viewMode, activeTab, doctors, suppliers, designers, orders, transactions, adjustments, showAllOrders, dateRange, getEffectiveDesignPrice]);
    ...
    const calculateOpeningBalance = useMemo(() => {
        ...
        return 0;
    }, [dateRange.start, selectedEntityId, activeTab, detailOrders, detailTransactions, orders, transactions, adjustments, childDoctorFilter, branchFilter, doctors, designers, showAllOrders, getEffectiveDesignPrice]);
    ...
    const individualStatement = useMemo(() => {
        ...
        if (activeTab === 'doctors') {
            ...
            const supOrders = relevantOrders.filter(o => !o.isDeleted && (isVisibleInAccountStatement(o) || showAllOrders) && o.supplierId === selectedEntityId && (showAllOrders || o.status === 'Delivered' || isDoctorRejectedStatus(o.status) || isLabRejectedStatus(o.status) || o.status === 'Cancelled'));
            ...
            const desOrders = relevantOrders.filter(o => !o.isDeleted && (isDesignerPayable(o) || showAllOrders) && o.designerId === selectedEntityId && o.workflowType === 'split' && (showAllOrders || o.designStatus === 'completed' || isDoctorRejectedStatus(o.status) || isLabRejectedStatus(o.status) || o.status === 'Cancelled'));
    ...
    }, [viewMode, selectedEntityId, activeTab, showAllOrders, dateRange, orders, transactions, adjustments, detailOrders, detailTransactions, calculateOpeningBalance, hiddenTransactionIds, childDoctorFilter, branchFilter, doctors, designers, getEffectiveDesignPrice]);
                                        ...
                                        return (
                                            <tr
                                                ...
                                                className={clsx(
                                                    ...
                                                    isZeroValueStatementItem(item) && "bg-red-50/70 text-red-800",
                                            ...
                                            >
                                                ...
                                                <td className="p-3">
                                                    <div className={clsx("font-medium", isZeroValueStatementItem(item) ? "text-red-700 line-through decoration-red-300" : "text-gray-800")}>{item.description}</div>
                                                    {item.details && <div className={clsx("text-xs mt-0.5 truncate max-w-xs", isZeroValueStatementItem(item) ? "text-red-400" : "text-gray-500")}>{item.details}</div>}
                        ...
                        visibleStatementItems.map(item => {
                            ...
                            return (
                                <article key={item.id} className={clsx(
                                    ...
                                    isZeroValueStatementItem(item) ? 'border-red-200 bg-red-50/60' : 'border-gray-200'
                                )}>
                                    <div className="flex items-start gap-3 p-3">
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-3">
                                                <div className="min-w-0">
                                                    <p className={clsx('text-sm font-black leading-6', isZeroValueStatementItem(item) ? 'text-red-700 line-through decoration-red-300' : 'text-gray-900')}>
                                                        {item.description}
                                                    </p>
