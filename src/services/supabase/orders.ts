...
export async function getOrders(
    ...
    if (filters.search && filters.search.trim()) {
        ...
        // 2. Build OR query: Use ilike for case_id to leverage database indexes
        const isAsciiSearch = /^[\w\s-]+$/.test(searchTerm);
        let orQuery = isAsciiSearch
            ? `case_id.ilike.%${searchTerm}%,patient_name.ilike.%${searchTerm}%`
            : `case_id.ilike.%${searchTerm}%,patient_name.imatch.${regex}`;
...
export async function getOrdersWithComments(): Promise<Order[]> {
    // Query recent comments directly from order_comments instead of downloading every order
    const { data: commentsData, error: commentsError } = await supabase
        .from('order_comments')
        .select('id, order_id, content, user_id, user_name, created_at')
        .order('created_at', { ascending: false })
        .limit(200);

    if (commentsError) {
        throw ErrorHandler.handle(commentsError, 'getOrdersWithComments');
    }

    if (!commentsData || commentsData.length === 0) return [];

    // Filter out auto-generated comments first
    const manualComments = commentsData
        .map(c => ({
            id: c.id,
            orderId: c.order_id,
            text: c.content,
    ...
    if (manualComments.length === 0) return [];

    const orderIds = Array.from(new Set(manualComments.map(c => c.orderId).filter(Boolean)));
    if (orderIds.length === 0) return [];

    // Fetch only the matching orders that are active
    ...
    if (ordersError) {
        throw ErrorHandler.handle(ordersError, 'getOrdersWithComments');
    }

    const orderMetaMap = new Map<string, { caseId: string; patientName: string }>();
    for (const o of ordersData || []) {
        orderMetaMap.set(o.id, { caseId: o.case_id, patientName: o.patient_name });
    }

    const orderMap = new Map<string, Order>();
    for (const comment of manualComments) {
        const meta = orderMetaMap.get(comment.orderId);
        if (!meta) continue;

        if (!orderMap.has(comment.orderId)) {
            orderMap.set(comment.orderId, {
                id: comment.orderId,
                caseId: meta.caseId,
            ...
            orderMap.get(comment.orderId)!.comments!.push(comment);
        }
    }
...
/**
 * Lightweight count of pending unregistered cases for navigation badges.
 * Replaces fetching the entire historical database of orders.
 */
export async function getUnregisteredCasesCount(): Promise<number> {
    const { data, error } = await supabase
        .from('orders')
        .select(`
            id,
            ...
            production_status,
            is_deleted,
            exclude_from_accounting_registration,
            order_comments(content)
        `)
        .or('is_deleted.eq.false,is_deleted.is.null')
        .not('exclude_from_accounting_registration', 'is', true)
        .or('is_registered.eq.false,is_registered.is.null,needs_accounting_reregistration.eq.true');

    if (error) {
        throw ErrorHandler.handle(error, 'getUnregisteredCasesCount');
    }
    ...
    interface CandidateRow {
        ...
        exclude_from_accounting_registration: boolean | null;
        order_comments?: { content: string }[] | null;
    }

    let count = 0;
    for (const row of (data || []) as unknown as CandidateRow[]) {
        const rowComments = Array.isArray(row.order_comments) ? row.order_comments : [];
        const orderCandidate = {
            id: row.id,
            status: row.status as Order['status'],
            issueState: row.issue_state as Order['issueState'],
            isRegistered: Boolean(row.is_registered),
            needsAccountingReregistration: Boolean(row.needs_accounting_reregistration),
            actualDeliveryDate: row.actual_delivery_date,
            firstDeliveredAt: row.first_delivered_at,
            productionStatus: row.production_status as Order['productionStatus'],
            isDeleted: Boolean(row.is_deleted),
            excludeFromAccountingRegistration: Boolean(row.exclude_from_accounting_registration),
            comments: rowComments.map((c: { content: string }) => ({ text: c.content } as NonNullable<Order['comments']>[number])),
        } as Order;

        if (isAccountingRegistrationCandidate(orderCandidate, 'pending')) {
            count++;
        }
    }
...
export async function getOrderIssues(filters?: {
    ...
    return (data || []).map((r) => {
        ...
        return {
            ...
            responsibleStage: r.responsible_stage ?? null,
            previousCauseCategory: r.previous_cause_category ?? null,
