...
export const wasRejected = (order: Order): boolean => {
    ...
    if (order.status === 'Cancelled' && order.statusHistory) {
        return order.statusHistory.some(h => h.status === 'Rejected');
    }
    return false;
};
...
export const getTechStatusBadge = (status?: string) => {
    switch (status) {
        ...
        case 'Rejected': return <span className="flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-50 dark:bg-red-900/30 px-1.5 py-0.5 rounded border border-red-100 dark:border-red-800" title="رفض من المعمل"><X size={10} /> رفض معمل</span>;
